...
switch (_type) do {
    case 0: { // IFAK
        ...
        _container = [(missionNamespace getVariable [KATQGVAR(misc,IFAK_Container), 0])] call _fnc_getContainer;
        _itemList = missionNamespace getVariable [KATQGVAR(misc,IFAKContents), []];
        _removeOnEmptyCondition = KATGVAR(misc,IFAK_RemoveWhenEmpty);
    };
    case 1: { // AFAK
        _FAKToAdd = "kat_AFAK_Magazine";
        _slotArray = [true,true,true,true,true,true,false,false];
        _container = [(missionNamespace getVariable [KATQGVAR(misc,AFAK_Container), 0])] call _fnc_getContainer;
        _itemList = missionNamespace getVariable [KATQGVAR(misc,AFAKContents), []];
        _removeOnEmptyCondition = KATGVAR(misc,AFAK_RemoveWhenEmpty);
    };
    case 3: { // M1
        _FAKToAdd = "kat_btc_FAK_M1_Magazine";
        _slotArray = [true,true,true,true,true,true,true,true];
        _container = [(missionNamespace getVariable [KATQGVAR(misc,M1_Container), 0])] call _fnc_getContainer;
        _itemList = missionNamespace getVariable [KATQGVAR(misc,M1Contents), []];
        _removeOnEmptyCondition = KATGVAR(misc,M1_RemoveWhenEmpty);
    };
    case 4: { // M2
        _FAKToAdd = "kat_btc_FAK_M2_Magazine";
        _slotArray = [true,true,true,true,true,true,true,true];
        _container = [(missionNamespace getVariable [KATQGVAR(misc,M2_Container), 0])] call _fnc_getContainer;
        _itemList = missionNamespace getVariable [KATQGVAR(misc,M2Contents), []];
        _removeOnEmptyCondition = KATGVAR(misc,M2_RemoveWhenEmpty);
    };
    default { // MFAK
        _FAKToAdd = "kat_MFAK_Magazine";
        _slotArray = [true,true,true,true,true,true,true,true];
        _container = [(missionNamespace getVariable [KATQGVAR(misc,MFAK_Container), 0])] call _fnc_getContainer;
        _itemList = missionNamespace getVariable [KATQGVAR(misc,MFAKContents), []];
        _removeOnEmptyCondition = KATGVAR(misc,MFAK_RemoveWhenEmpty);
    };
};
...
if (_itemType == "Item") exitWith {
    ...
    if (_slot > 0) then {
        ...
        [_unit, _FAKToAdd, "", ([_slotArray] call KATFUNC(misc,FAK_arrayToAmmo))] call ACEFUNC(common,addToInventory);
...
private _ammoCount = [_unit, _item, true] call KATFUNC(misc,getMagazineAmmoCounts);
private _lowestAmmoCount = 257;
private _containerFAK = "";
{
    private _checkedArray = [(_x select 0), _type] call KATFUNC(misc,FAK_ammoToArray);
...
if (_slot > 0) then {
    ...
    private _remaining = [_slotArray] call KATFUNC(misc,FAK_arrayToAmmo);
